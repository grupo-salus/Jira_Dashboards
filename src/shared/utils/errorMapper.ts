...
const ERROR_MAPPINGS: ErrorMapping[] = [
  ...
  },
  {
    ...
    message:
      "Servidor não está respondendo. Tente novamente em alguns instantes.",
    type: "network",
  ...
  },
  {
    pattern: /503/,
    message:
      "Serviço temporariamente indisponível. Estamos fazendo manutenção.",
    type: "server",
    code: "SERVER_503",
  },
  {
...
// Função auxiliar para testar padrões
const testPattern = (pattern: string | RegExp, text: string): boolean => {
...
export const mapErrorMessage = (error: unknown): string => {
  ...
  for (const mapping of ERROR_MAPPINGS) {
    if (testPattern(mapping.pattern, errorString)) {
...
export const getErrorType = (
  error: unknown
): "network" | "server" | "auth" | "data" | "unknown" => {
  const errorString = error instanceof Error ? error.message : String(error);
