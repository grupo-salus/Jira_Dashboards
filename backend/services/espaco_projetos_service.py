...
class Projeto:
    ...
    responsavel_atual: Optional[str] = None  # Novo campo
    relator: Optional[str] = None
    squad: Optional[str] = None
    squads: Optional[List[str]] = None  # Novo campo - lista de squads
    ...
    status_fase_atual: Optional[str] = None  # Novo campo
...
class ProjetoParser:
    ...
    def parse_issue(self, issue: Dict[str, Any]) -> Projeto:
        ...
        return Projeto(
            ...
            responsavel_atual=fields.get("customfield_10413", {}).get("value"),
            ...
            motivo_cancelamento=fields.get("customfield_10412", {}).get("value"),
            motivo_repriorizacao=fields.get("customfield_10542", {}).get("value")
        )
...
class ProjetoAnalisador:
    ...
    def __init__(self):
        ...
        self.fases = [
            "ideacao", "bloqueado", "backlog_priorizado", "analise_tecnica_e_negocios", "em_desenvolvimento",
            "em_homologacao", "operacao_assistida", "entregue", "cancelado"
        ]
    ...
    def analisar_projeto(self, projeto: Projeto) -> Projeto:
        ...
        projeto = self._calcular_status_fase_atual(projeto)
    ...
    def _calcular_posicao_backlog(self, projeto: Projeto) -> Projeto:
        ...
        # Esta função será chamada individualmente para cada projeto
        # A lógica de posicionamento por responsável será aplicada no serviço principal
        if (projeto.status == "Backlog priorizado" or 
            projeto.data_inicio_backlog_priorizado is not None):
            projeto.posicao_backlog = None  # Será calculado posteriormente
        else:
    ...
    def _calcular_status_fase_atual(self, projeto: Projeto) -> Projeto:
        """Calcula o status da fase atual do projeto baseado no status atual e nas datas de início/fim da fase."""
        status = projeto.status.lower().replace(" ", "_")
        status_validos = ["em_desenvolvimento", "em_homologacao", "operacao_assistida", "analise_tecnica_e_negocios", "backlog_priorizado"]
        
        if status not in status_validos:
            projeto.status_fase_atual = None
            return projeto
        
        # Verificar se o projeto foi repropriado
        if projeto.motivo_repriorizacao and projeto.posicao_backlog is not None and projeto.posicao_backlog != 1:
            projeto.status_fase_atual = "Repriorizado"
            return projeto
        ...
        inicio = getattr(projeto, inicio_attr, None)
        fim_previsto = getattr(projeto, fim_attr, None)
        
        if inicio is None:
            projeto.status_fase_atual = "Não iniciado"
            return projeto
        
        if fim_previsto is not None:
            dias_restantes = self._dias_uteis_entre_datas(self.hoje, fim_previsto)
            if dias_restantes < 0:
                projeto.status_fase_atual = "Atrasado"
            elif dias_restantes <= 2:
                projeto.status_fase_atual = "Em risco"
            else:
                projeto.status_fase_atual = "No prazo"
        else:
            dias_decorridos = self._dias_uteis_entre_datas(inicio, self.hoje)
            projeto.status_fase_atual = "Em desenvolvimento"
...
class ProjetoSerializer:
    ...
    def to_dict(projeto: Projeto) -> Dict[str, Any]:
        ...
        return {
            ...
            "Status da fase atual": projeto.status_fase_atual,
